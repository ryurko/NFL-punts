---
title: "NFL Punt Win Probability Impact"
...
# Proposed rule change win probability impact
...
proposed incentives on the game itself. We propose the following three incentives:

#. Return team gain 5 additional yards on a fair catch.
#. Punting team pins return team 5 yards further back from point the ball goes out of bounds in air.
#. Return team starts at 15 yard line instead of 20 yard line on touchbacks from 
punts that landed/crossed in the endzone __through the air__. This is to promote
punters booming punts into the endzone that are not returnable.

There are no proposed incentive structures for punts that land in play (in bounds
and not in the endzone), regular rules will apply. This means that punts that 
roll into the endzone will be touchbacks that start at the normal 20 yard line.
...
a reference line at our proposed shift of five yards.
...
punt_pbp_data %>%
  ...
  labs(x = "Return yards",
       y = "Frequency", 
       title = "Distribution of punt return yards",
       subtitle = "All punt returns from pre-, post-, and regular season games in 2016 and 2017",
       caption = "Data courtesy of NFL.com, accessed with `nflscrapR`") +
  theme_bw() 
```
...
punt_pbp_data %>%
  ...
  labs(x = "Return yards",
       ...
       title = "Empirical CDF of punt return yards",
       subtitle = "All punt returns from pre-, post-, and regular season games in 2016 and 2017",
       caption = "Data courtesy of NFL.com, accessed with `nflscrapR`") +
  theme_bw() 
```
...
win probability added from the proposed change for all punt returns with
fair catch, out of bounds, or touchback outcomes and compare it to the win probability
...
rule change. We are not included downed punts in this analysis.
...
punt_adj_data <- punt_pbp_data %>%
  ...
  # Remove downed punt rows:
  filter(punt_type != "downed") %>%
...
punt_adj_data %>%
  ...
  mutate(punt_type = fct_relevel(punt_type, "return", "fair_catch", 
         ...
         punt_type = fct_recode(punt_type, `Regular punt returns` = "return", 
                                              `Rule change: fair catch` = "fair_catch",
                                              `Rule change: touchback` = "touchback",
                                              `Rule change: out of bounds` = "out_of_bounds")) %>%
